...
const makeRankings = (googleData, predictHQ, interests, dislikes) => {
  const possibilities = googleData
    .filter(event => {
      for (let i = 0; i < event.types.length; i++) {
        if (
          interests.includes(event.types[i]) &&
          event.name !== 'Walking Tours' &&
          !dislikes.includes(event.types[i])
        ) {
          return event;
        }
      }
  ...
  sortedByDistance = sortedByDistance.sort(
    (a, b) => a.distanceFromTopRated - b.distanceFromTopRated
  );
...
const fillDay = (day, rankedList, interests, restaurants) => {
  ...
  while (timeSpent <= 12 && currentEvent < rankedList.length) {
    day[`event${++eventsPlaced}`] = {
      ...
      location: {
        latitude: rankedList[currentEvent].geometry.location.lat,
        longitude: rankedList[currentEvent].geometry.location.lng
      },
  ...
  while (restaurantsPlaced < 5) {
    day[`restaurant${++restaurantsPlaced}`] = findRestaurant(
      rankedList[0],
      restaurantsPlaced,
      restaurants
    );
...
const scheduleBuilder = (
  ...
  days.forEach(day => {
    schedule[day].events = sorted.splice(
      0,
      Math.floor(googleLength / (numberOfDays + 1))
    );
    ...
    for (let i = 0; i < 4; i++) {
      schedule[day].restaurants.push(
        findRestaurant(schedule[day].events[0], 0, restaurantData)
      );
    }
...
const getSchedule = (startDate, endDate, location, interests, cb) => {
  ...
  axios
    .get(
      `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${query}+point+of+interest&language=en&key=${
        keys.googlePlacesAPI
      }`
    )
    .then(response1 => {
      ...
      return axios.get(
        `https://maps.googleapis.com/maps/api/place/textsearch/json?pagetoken=${
          response1.data.next_page_token
    ...
    .then(response2 => {
      ...
      return axios.get(
        `https://maps.googleapis.com/maps/api/place/textsearch/json?pagetoken=${
          response2.data.next_page_token
        }&key=${keys.googlePlacesAPI}`
      );
    })
    .then(response3 => {
      ...
      return Promise.all([
        axios.get(
          `https://api.predicthq.com/v1/events/?category=concerts,festivals,performing-arts,sports&within=10mi@${
            googleData[0].geometry.location.lat
          },${
            googleData[0].geometry.location.lng
          }.0060&start.gte=2018-02-11&start.lte=2018-02-17&rank_level=4,5`,
          predictHQconfig
        ),
        axios.get(
          `https://developers.zomato.com/api/v2.1/search?lat=${
            googleData[0].geometry.location.lat
          }&lon=${googleData[0].geometry.location.lng}&sort=rating`,
          zomatoConfig
        )
      ]);
    })
